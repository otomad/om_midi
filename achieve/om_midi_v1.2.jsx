 ...
 * Look forward to seeing your name after us.
 * 
 * ****************************************
 * I'm not an otomader or scripter, just edit it for fun.
 * Without OMINO and NGDXW, you can't see this script.
 * -----
 * I believe free and open entertainment.
 * 我相信自由与开放的娱乐
 * --Z4HD
 * ****************************************
 * 
...
function addMidiFileMethods(m) {
	...
	m.getTrackByChannel = function (chObj) {
		//return Track obj
		var result;
		result = this.tracks[chObj.trackIndex];
		return result
	}
 ...
 * Useage: 
 * 	By key: '{name} is {type}'.format({"name":"GO","type":"DUST"})
 *  By index : '{0} is {1}'.format('GO','GOD')
*/
function fmt(args) {
	var result = this;
	if (args.length > 0) {
...
﻿{
	...
	var D_CONTROLLABELWIDTH = 92;
	var D_CONTROLWIDTH = 200;// edited Window width
	...
	function _odFileCommon(label, name, path, prompt, extension) {
		...
		var button = oD.w.add('button', buttonBox, '浏览...');
		...
		button.onChange = function () {
			...
			//this.file.openDialog('选择一个MIDI文件',"MIDI files:*.mid;All files:*.*")//已写死，过滤MIDI文件。
			this.nameCtl.text = decodeURI(this.file.name);
			//MIDI_FILE_NAME = this.file.name;
		}
		return button;
	}
	...
	function _odOpenFile(label, name, path, prompt, extension)//打开文件
	{
		...
		buttonCtl.onClick = function () {
			var f = this.file.openDlg(this.filePrompt, extension);
			//   alert(f)
			if (f) {
				this.file = f;
				this.value = f.fsName;
				this.nameCtl.text = decodeURI(f.name);
				MIDI_FILE_NAME = decodeURI(f.name); // 刷新全局 MIDI 文件名称
			}
		};
		return buttonCtl;
	}
	...
	function newOminoDialog(name, existingPanel, paletteCallback, paletteCallbackArg2) {
		...
		var oD = new Object();
		oD.w = existingPanel ? existingPanel : new Window(kind, name, [200, 200, 500, 500], { resizeable: true });
...

{
	...
	function applyChannelToLayer(channel, layer, pb, totalNotesCount, notesSoFar, channelName)//AAAAA
	{
		...
		//var nameTemplate1 = "ch{0}_[{1}]_{2}";
		//var name = nameTemplate1.format(midiChannel,channelName,"{0}");
		var name = 'CH' + midiChannel + '_[' + channelName + ']_'
		// 汉化来源：AV9228581
		pitchSliderName = name + "音高(pitch)";
		velSliderName = name + "力度(vel)";
		durSliderName = name + "持续时间(dur)";
		/*这里开始改动*/
		scaleSliderName = name + "缩放";/*新增*/
		advScaleSliderName = name + "高级缩放";/*新增*/
		...
		addSliderControl(layer, advScaleSliderName);
		addSliderControl(layer, rotationCSliderName);/*新增*/
		addSliderControl(layer, rotationASliderName);/*新增*/
		...
		var advScaleTimes = new Array();
		var advScaleValues = new Array();
		...
		for (var i = 0; i < channel.notes.length; i++) {
			...
			if (vel && typeof (dur) != "undefined") {
				...
				advScaleTimes.push(t);
				advScaleValues.push(ijk % 2 == 0 ? 1 : -1);
		...
		var scaleValue = layer.Effects.property(scaleSliderName).property(1);/*新增*/
		var advScaleValue = layer.Effects.property(advScaleSliderName).property(1);
		var rotationCValue = layer.Effects.property(rotationCSliderName).property(1);/*新增*/
		var rotationAValue = layer.Effects.property(rotationASliderName).property(1);/*新增*/
		var countValue = layer.Effects.property(countSliderName).property(1);/*新增*/
		var boolValue = layer.Effects.property(boolSliderName).property(1);/*新增*/
		var tValue = layer.Effects.property(tSliderName).property(1);/*新增*/
		...
		if (scaleTimes.length > 0)/*新增*/
			scaleValue.setValuesAtTimes(scaleTimes, scaleValues);/*新增*/
		setAllKeyFrames(scaleValue, KeyframeInterpolationType.HOLD);/*新增*/

		if (advScaleTimes.length > 0)
			advScaleValue.setValuesAtTimes(advScaleTimes, advScaleValues);
		setAllKeyFrames(advScaleValue, KeyframeInterpolationType.HOLD);


		if (rotationCTimes.length > 0)/*新增*/
			rotationCValue.setValuesAtTimes(rotationCTimes, rotationCValues);/*新增*/
		setAllKeyFrames(rotationCValue, KeyframeInterpolationType.HOLD);/*新增*/

		if (rotationATimes.length > 0)/*新增*/
			rotationAValue.setValuesAtTimes(rotationATimes, rotationAValues);/*新增*/
		setAllKeyFrames(rotationAValue, KeyframeInterpolationType.HOLD);/*新增*/

		if (countTimes.length > 0)/*新增*/
			countValue.setValuesAtTimes(countTimes, countValues);/*新增*/
		setAllKeyFrames(countValue, KeyframeInterpolationType.HOLD);/*新增*/

		if (boolTimes.length > 0)/*新增*/
			boolValue.setValuesAtTimes(boolTimes, boolValues);/*新增*/
		setAllKeyFrames(boolValue, KeyframeInterpolationType.HOLD);/*新增*/

		if (tTimes.length > 0)/*新增*/
			tValue.setValuesAtTimes(tTimes, tValues);/*新增*/
		setAllKeyFrames(tValue, KeyframeInterpolationType.LINEAR);/*新增*/
	...
	function applyMidiToLayer(midi, layer, totalNotesCount)//AAAAA
	{
		...
		var notesSoFar = 0;
		var pb = progressBar("正在分析： " + midi.filePath + "\n如果MIDI文件的轨道数较多则脚本可能暂时失去响应，请耐心等待。");
		pb.t1.preferredSize = [50, 3]
		for (var chKey in midi.channels) {
			var channel = midi.channels[chKey];
			var channelT = typeof channel;
			if (channelT != 'function') {
				tr = midi.getTrackByChannel(channel)
				applyChannelToLayer(channel, layer, pb, totalNotesCount, notesSoFar, tr.name);
	...
	function go(thisObj) {
		var omd = newOminoDialog("om_midi - NGDXW edited " + RELEASE_VERSION, thisObj, apply);
		var boxText = "读取一个MIDI文件，并为当前合成添加一个新图层，其中包含每个MIDI通道的音高，速度和持续时间等的滑块控件。\n\n由 NGDXW先辈修改  Z4HD汉化"
		omd.boxedText(7, boxText);

		omd.openFile("MIDI文件", "midiFileName", "Empty...", "选择一个MIDI文件", Array("MIDI files:*.mid", "All Files:*.*"));
	...
	function apply(result)//AAAAA2
	{
		...
		if (midi.notes.length > 0) {
			...
			var errorMsg = "无法读取到MIDI音轨\n\n请检查您选择的MIDI文件是否正确。";
			alert(errorMsg, '无法读取MIDI文件', true);
		}
