...
/// <reference path="Illustrator Tool List.d.ts" />
/// <reference path="AiMenu_2021.d.ts" />

/**
 * Element-placement instructions for moving items in stacking order containers.
 */
declare enum ElementPlacement {
  /**
   * Place inside the relative container-type item (Groups/Layers).
   */
  INSIDE = 0,

  /**
   * Place at the beginning (make first) of items inside the relative container-type item (Groups/Layers).
   */
  PLACEATBEGINNING = 1,

  /**
   * Place at the end (make last) of items inside the relative container-type item (Groups/Layers).
   */
  PLACEATEND = 2,

  /**
   * Place before (above in layer stacking order) the relative item.
   */
  PLACEBEFORE = 3,

  /**
   * Place after (below in layer stacking order) the relative item.
   */
  PLACEAFTER = 4,
}
...
/**
 * Denotes the purpose of color conversion using ConvertSampleColor method.
 */
...
declare class PageItems extends Array<PageItem> {
   ...
   * The class name of the object.
   */
  readonly typename: "GroupItem" | "PathItem" | "CompoundPathItem" | "TextFrame" | "SymbolItem" | "RasterItem" | "PlacedItem" | "EmbeddedItem" | "MeshItem" | "GraphItem" |
  "PluginItem" | "NonNativeItem" | "GridRepeatItem" | "RadialRepeatItem" | "SymmetryRepeatItem" | "LegacyTextItem";
...
declare class TextFonts extends Array<TextFont> {
  ...
  /**
   * Get the Text Font with the font name , if not available it will create the substitute font.
...
declare class GridRepeatItems extends Array<GridRepeatItem> {
   ...
   * @param {GridRepeatConfig} config - The grid repeat configuration.
   */
...
declare class RadialRepeatItems extends Array<RadialRepeatItem> {
   ...
   * @param {any} sourceArt - The page item(s) to apply to.
   * @param {RadialRepeatConfig} config - The radial repeat configuration.
   */
...
declare class SymmetryRepeatItems extends Array<SymmetryRepeatItem> {
   ...
   * @param {any} sourceArt - The page item(s) to apply to.
   * @param {SymmetryRepeatConfig} config - The symmetry repeat configuration.
   */
...
declare class OpenOptions {
  ...
  /**
   * Create Artboard with dimensions of artwork bounding box when opening legacy document (pre-Illustrator CS4) in CS4 or later.
   */
 ...
 */
declare class Color {
  typename: "LabColor" | "RGBColor" | "CMYKColor" | "GrayColor" | "NoColor" | "SpotColor" | "PatternColor" | "GradientColor";
}
...
declare class Application {
   ...
   */
  readonly name: "Adobe Illustrator"
  ...
  /**
   * Get the active session for the sequencer.
   */
   ...
   */
  executeMenuCommand(menuCommandString: AiExecuteMenuCommandStrings): void
   ...
   */
	selectTool (toolName: IllustratorToolNameStrings): boolean 
...
declare class Document {
  ...
  /**
   * If it's a large-scale document, this number is 10. For normal documents it's 1.
   */
  scaleFactor: 1 | 10;
  ...
  /**
   * Gets a pageItem from the document by its `.uuid` property.
   */
...
declare class PageItem {
  /**
   * The unique document-specific identifier assigned to this artwork item.
   */
  uuid: string
   ...
   * The class name of the object.
   */
  readonly typename: PageItemTypenames
  ...
  /**
   * Place art object(s)in perspective grid at specified perspective plane and coordinate.
...
declare class GridRepeatItem extends PageItem {
   ...
   */
  setGridConfiguration (config: GridRepeatConfig, state: GridRepeatUpdate)
}
...
declare class RadialRepeatItem extends PageItem {
   ...
   * Grid repeat config for repeat art.
   */
  readonly gridConfig: RadialRepeatConfig
   ...
   */
  setGridConfiguration (config: RadialRepeatConfig, state: RadialRepeatUpdate)
}
...
declare class SymmetryRepeatItem extends PageItem {
   ...
   * Grid repeat config for repeat art.
   */
  readonly gridConfig: SymmetryRepeatConfig
   ...
   */
  setGridConfiguration (config: SymmetryRepeatConfig, state: SymmetryRepeatUpdate)
}
...
declare class GridRepeatConfig {
   ...
   */
  columnFlipType: RepeatGridShiftFlipType;
   ...
   */
  patternType: RepeatGridShiftFlipType;
  
  /**
   * Row flip type of grid repeat art.
   */
  rowFlipType: RepeatGridShiftFlipType;
...
declare class SymmetryRepeatConfig {
   ...
   */
  axisRotationAngleInRadians: number;
}
