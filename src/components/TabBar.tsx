	...
	render() {
		...
		return (
			<>
				...
				<div className="indicator" ref={this.indicatorRef}></div>
			</>
		);
	}
}
...
function Li(props: HTMLProps<HTMLLIElement>) {
	return <li {...props} className={classNames([props.className, "ripple-button", "ripple-lighter", "button-like"])} tabIndex={0}>{props.children}</li>;
}
